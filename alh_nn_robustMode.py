...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  7 14:54:52 2018
...
class aerosolLayerHeight(Model):
    ...
    def __init__(self, prior=None, priorCov=None, limits=None, otherModelParam=None, 
        ...
        self.asfc_unc   = kwargs['asfc_uncertainty']
        ...
        self.randASFC = []
        self.guessd_multiple_alh = []
        self.guessd_multiple_aot = []
        
        if self.asfc > 0.05:
            # for land use absolute errors
            asfc_s = np.array([self.asfc-0.5*self.asfc_unc,self.asfc-0.25*self.asfc_unc, 
                               self.asfc,
                               self.asfc+0.25*self.asfc_unc, self.asfc+0.5*self.asfc_unc])
        else:
            # for ocean use relative errors of max 30%
            asfc_s = np.array([self.asfc-0.3*self.asfc,self.asfc-0.15*self.asfc, 
                               self.asfc,
                               self.asfc+0.15*self.asfc, self.asfc+0.3*self.asfc])

        self.randASFC = asfc_s
        ...
        self.NN_K_zaer   = import_model_fr(os.path.join(self.modelPath, self.models[1]), [100, 100, 100], tf.nn.sigmoid)    # for derivative w.r.t alh
        ...
        self.NN_K_as_758 = import_model_fr(os.path.join(self.modelPath, self.models[3]), [100, 100], tf.nn.sigmoid)
        self.NN_K_as_770 = import_model_fr(os.path.join(self.modelPath, self.models[4]), [100, 100], tf.nn.sigmoid)
    ...
    def guess_apriori(self):
        ...
        # guess a priori alh for multiple asfc values:
    
        inp_ap_alh = copy.deepcopy(inp_ap)
        guessd_multiple_alh = []
        guessd_multiple_aot = []
        for i in self.randASFC:
            
            inp_ap_alh[5] = i
            guessd_multiple_alh.append(list(self.NN_ap__zaer.run(inp_ap_alh))[0])
            guessd_multiple_aot.append(list(self.NN_ap__aot.run(inp_ap_alh))[0])
        
        self.guessd_multiple_alh = guessd_multiple_alh
        self.guessd_multiple_aot = guessd_multiple_aot
    ...
    def __call__(self, do_Jacobian=True):
        ...
        if iter__z_aer > self.psfc:
            
            self.z_aer = self.psfc - 200*random.uniform(0.8, 1.2)
            ...
            self.adjustedStateVector = False
        ...
        if ( val > self.minOptThicknessAer ) and ( val < self.maxOptThicknessAer ):
            ...
            elif val > self.maxOptThicknessAer:
        ...
        if self.dynamicScaling:
            ...
            logical_ds = M > threshold  # original
...
class NNoutput(tables.IsDescription):
    ...
    ret_alh = tables.Float32Col(shape=(5,))
    ret_alh_km = tables.Float32Col(shape=(5,))
    ret_aot = tables.Float32Col(shape=(5,))
    
    apriori_alh = tables.Float32Col(shape=(5,))
    apriori_aot = tables.Float32Col(shape=(5,))
    varied_asfc = tables.Float32Col(shape=(5,))
    
    wrmse = tables.Float32Col(shape=(5,))
    aPosErrCovMat = tables.Float32Col(shape = (2, 2, 5))
    chi2 = tables.Float32Col(shape=(5,))
    dof = tables.Float32Col(shape=(5,))
    numiter = tables.Float32Col(shape=(5,))
    svConvCrit = tables.Float32Col(shape=(5,))
    converged = tables.Int32Col(shape=(5,))
    oeTime = tables.Float32Col(shape=(5,))
...
#    lon_ur = -130
    ...
    try:
        AAI         = glob.glob(os.path.join(orbit_path,'S5P_OFFL_L2__AER_AI*.nc'))[0]
    except Exception as e:
        AAI         = glob.glob(os.path.join(orbit_path,'S5P_TEST_L2__AER_AI*.nc'))[0]
    ...
    try:
        ...
        tropl2out = glob.glob(os.path.join(orbit_path,'S5P_TEST_L2__ALH*.nc'))[0]
    ...
    with tables.open_file( LER, 'r' ) as f:
        ...
        uncertaintyData = f.root._f_get_child('Uncertainty')[:]
    
    tree, a, b, t, sp, lat_tp, lon_tp, time_data = obtainTempPresProfile_fromMeteoData.assemble_tree(meteofile, meteofile_sp)
    
    fr_modelVersion = 'v1'
    ap_modelVersion = 'v0'
    modelpath = os.path.join('./', 'models', 'forward_model', fr_modelVersion)
    modelpathAP = os.path.join('./', 'models', 'apriori_model', ap_modelVersion)
    outputname = 'retrievalResults_NNmodel_robustMode__{}.h5'.format(fr_modelVersion)
...
#    pixels_to_process = list(zip(mask_o2a[0], mask_o2a[1]))
    ...
    # from scl pxl range
#    scl = np.arange(1500, 1700)
#    pxl = np.arange(0, 420)
#    pixels_to_process = [(i,j) for i in scl for j in pxl]
    
    # from lat lon bounds
    with nc.Dataset(filenameRad, 'r') as f:
        latc = f['BAND{0}_RADIANCE'.format(int(6))]['STANDARD_MODE']['GEODATA']['latitude'][0,:,:]
        lonc = f['BAND{0}_RADIANCE'.format(int(6))]['STANDARD_MODE']['GEODATA']['longitude'][0,:,:]
    
    logical_lat = (latc > min(lat_ur, lat_ll)) & (latc < max(lat_ur, lat_ll))
    logical_lon = (lonc > min(lon_ur, lon_ll)) & (lonc < max(lon_ur, lon_ll))
    
    latPos = np.argwhere(logical_lat)
    lonPos = np.argwhere(logical_lon)
    
    latin = [tuple(i) for i in latPos.tolist()]
    lonin = [tuple(i) for i in lonPos.tolist()]
    pixels_to_process = list(set(latin).intersection(lonin))
    
    print('pixels within geographical bounds:', len(pixels_to_process))
    
    # AAI filter
    with nc.Dataset(AAI, 'r') as f:
        aai = f['PRODUCT']['aerosol_index_340_380'][0,:,:]
        aai_selection = [aai[i[0],i[1]] for i in pixels_to_process]
    
    aai_logical = np.array(aai_selection) >= 1.0

    # if sorting by AAI:
    pixels_to_process = np.array(pixels_to_process)
    pixels_to_process = pixels_to_process[aai_logical,:]
    pixels_to_process = pixels_to_process.tolist()
    ...
    print('total number of pixels to process:', pixels_to_process.shape[0])
    ...
    models = ['I__o2a', 'K_alp', 'K_aot', 'K_as758', 'K_as770']
    modelsAP = ['ap__zaer', 'ap__aot']

    count = pixels_to_process.shape[0]
    gapriori = False                 # guess a priori parameters
    dynamicScalingSwitch = True      # to be used in robust mode
    fixASFC = True                   # to be used if ocean as is incorrec
        ...
        for selPxl in pixels_to_process:
            ...
            with tables.open_file(outputfilename,
                ...
                salb758, salb772 = extractsurfacealbedo.extractsurfalbedo_lite(dataLat, dataLon, data_wavelengths, lerData, month,[758., 772],
                                                                          lat,lon)
                unc758, unc772 = extractsurfacealbedo.extractsurfalbedoUncertainty_lite(dataLat, dataLon, data_wavelengths, uncertaintyData, month,[758., 772],
                                                                          lat,lon)
                ...
                print(repr(instrWavl))
                sys.exit(1)
                
                if (fixASFC) and (asfc[0]<0.06):
                    asfc = [0.022457076, 0.022457076]
                ...
                if gapriori:
                    ...
                    multi_alh = oe.model.guessd_multiple_alh[:]
                    multi_aot = oe.model.guessd_multiple_aot[:]
                    multi_asfc = oe.model.randASFC[:]
                
                else:

                    multi_alh = np.ones((5,))*priori[0]
                    multi_aot = np.ones((5,))*priori[1]
                    multi_asfc = oe.model.randASFC[:]                    
                ...
                oeTime = []
                converged = []
                aPosErrCovMat = np.ones((2,2,5))
                
                for idx, item in enumerate(multi_alh):
                    
                    oe.model.guess_apriori = False
                    prior_sv = [multi_alh[idx], multi_aot[idx]]
                    oe.model.initialStateVector = prior_sv
                    oe.model.asfc = multi_asfc[idx]
                    
                    startTime = time.time()
                    oe()
                    stoptime = time.time()
                    oeTime.append(stoptime - startTime)
                    print('time taken for retrieval:', oeTime[-1], 'seconds')
                    print('pixels left: ', count)
                    
                    if oe.answer['converged']:
                        converged.append(1)
                    else:
                        converged.append(0)
                    
                    wrmse.append(oe.answer['weighted root mean square difference'])
                    chi2.append(oe.answer['cost function'])
                    dof.append(oe.answer['degrees of freedom'])
                    numiter.append(oe.answer['number of iterations'])
                    aPosErrCovMat[:,:,idx] = oe.answer['a posteriori noise error covariance matrix'] 
                    svConvCrit.append(oe.answer['state vector convergence criterium'])
                    ret_alh.append(oe.answer['a posteriori state vector'][0])
                    ret_aot.append(oe.answer['a posteriori state vector'][1])
...
#%%
                ...
                ret_alh_km = [oe.model.descale_hPa(rethpa) for rethpa in ret_alh]
                output['ret_alh_km'] = ret_alh_km
                output['ret_aot'] = ret_aot    
                
                output['apriori_alh'] = multi_alh
                output['apriori_aot'] = multi_aot
                output['varied_asfc'] = multi_asfc
                ...
                print('retrieved alh(s) in km:', repr(ret_alh_km))
